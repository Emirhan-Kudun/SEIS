...
export const metadata: Metadata = {
  title: "Cloud Chaos Lab",
  description: "Cloud chaos command surface for experiment cadence, blast-radius governance, and postmortem linkage.",
  alternates: { canonical: "/cloud-chaos-lab" }
};

type CloudChaosLabPageProps = {
  searchParams?: { lang?: string };
};

export default function CloudChaosLabPage({ searchParams }: CloudChaosLabPageProps) {
  const locale = resolveLocale(searchParams?.lang);
  const summary = getCloudChaosSummary();
  const signals = getCloudChaosSignals();
  const tasks = getCloudChaosTasks();
  ...
  return (
    <main className="min-h-screen bg-seis-bg text-seis-text">
      <section className="mx-auto w-[min(1120px,calc(100vw-1.5rem))] py-12 sm:py-16">
        <p className="text-xs uppercase tracking-[0.14em] text-seis-accent">SEIS Cloud Chaos Lab</p>
        <h1 className="mt-3 max-w-4xl font-serif text-4xl leading-tight sm:text-5xl">
          Chaos engineering governance with controlled blast-radius and measurable resilience hypotheses.
        </h1>
        <p className="mt-4 max-w-3xl text-sm text-seis-muted sm:text-base">
          Reduces surprise incidents by converting chaos learning into explicit operational controls.
        </p>
        ...
        <div className="mt-6 grid gap-3 lg:grid-cols-2">
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Chaos Signals</h2>
          ...
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Chaos Tasks</h2>
            <ul className="mt-3 grid gap-2 text-sm text-seis-muted">
              {tasks.map((item) => (
                <li key={item.id} className="rounded border border-seis-line bg-[#1a1510] px-3 py-3">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Link href="/api/cloud-chaos-command" className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">API: /api/cloud-chaos-command</Link>
          ...
          <Link href={`/research-lab?lang=${locale}`} className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">Open research lab</Link>
