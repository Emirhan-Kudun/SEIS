...
export const metadata: Metadata = {
  title: "Cloud Release Train Lab",
  description: "Cloud release-train command surface for cadence, gate strictness, handoff quality, and readiness continuity.",
  alternates: { canonical: "/cloud-release-train-lab" }
};

type CloudReleaseTrainLabPageProps = {
  searchParams?: { lang?: string };
};

export default function CloudReleaseTrainLabPage({ searchParams }: CloudReleaseTrainLabPageProps) {
  const locale = resolveLocale(searchParams?.lang);
  const summary = getCloudReleaseTrainSummary();
  const signals = getCloudReleaseTrainSignals();
  const tasks = getCloudReleaseTrainTasks();
  ...
  return (
    <main className="min-h-screen bg-seis-bg text-seis-text">
      <section className="mx-auto w-[min(1120px,calc(100vw-1.5rem))] py-12 sm:py-16">
        <p className="text-xs uppercase tracking-[0.14em] text-seis-accent">SEIS Cloud Release Train Lab</p>
        <h1 className="mt-3 max-w-4xl font-serif text-4xl leading-tight sm:text-5xl">
          Cloud release cadence and gating controls with rollback-aware handoff discipline.
        </h1>
        <p className="mt-4 max-w-3xl text-sm text-seis-muted sm:text-base">
          Keeps release waves predictable by turning gate policy and handoff quality into explicit signals.
        </p>
        ...
        <div className="mt-6 grid gap-3 lg:grid-cols-2">
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Release Train Signals</h2>
          ...
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Release Train Tasks</h2>
            <ul className="mt-3 grid gap-2 text-sm text-seis-muted">
              {tasks.map((item) => (
                <li key={item.id} className="rounded border border-seis-line bg-[#1a1510] px-3 py-3">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Link href="/api/cloud-release-train-command" className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">API: /api/cloud-release-train-command</Link>
          ...
          <Link href={`/handoff-lab?lang=${locale}`} className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">Open handoff lab</Link>
