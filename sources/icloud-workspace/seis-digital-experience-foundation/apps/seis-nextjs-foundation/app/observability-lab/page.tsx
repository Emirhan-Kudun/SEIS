...
} from "@/lib/observability-command";

export const metadata: Metadata = {
  title: "Observability Lab",
  description: "Observability command surface for metrics, logs, traces, and alert governance.",
  alternates: {
    canonical: "/observability-lab"
  }
};

type ObservabilityLabPageProps = {
  searchParams?: { lang?: string };
};

export default function ObservabilityLabPage({ searchParams }: ObservabilityLabPageProps) {
  const locale = resolveLocale(searchParams?.lang);
  const summary = getObservabilitySummary();
  const signals = getObservabilitySignals();
  const tasks = getObservabilityTasks();
  ...
  return (
    <main className="min-h-screen bg-seis-bg text-seis-text">
      <section className="mx-auto w-[min(1120px,calc(100vw-1.5rem))] py-12 sm:py-16">
        <p className="text-xs uppercase tracking-[0.14em] text-seis-accent">SEIS Observability Lab</p>
        <h1 className="mt-3 max-w-4xl font-serif text-4xl leading-tight sm:text-5xl">
          Runtime visibility layer for command APIs, alerts, and cross-system telemetry confidence.
        </h1>
        <p className="mt-4 max-w-3xl text-sm text-seis-muted sm:text-base">
          Aligns metrics, traces, and escalation signals to reduce operational blind spots.
        </p>
        ...
        <div className="mt-6 grid gap-3 lg:grid-cols-2">
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Observability Signals</h2>
          ...
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Observability Tasks</h2>
            <ul className="mt-3 grid gap-2 text-sm text-seis-muted">
              {tasks.map((item) => (
                <li key={item.id} className="rounded border border-seis-line bg-[#1a1510] px-3 py-3">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Link href="/api/observability-command" className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">API: /api/observability-command</Link>
          ...
          <Link href={`/sre-lab?lang=${locale}`} className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">Open SRE lab</Link>
