...
export const metadata: Metadata = {
  title: "Merge Lab",
  description: "Merge command surface for conflict visibility, sequence governance, and review discipline.",
  alternates: { canonical: "/merge-lab" }
};

type MergeLabPageProps = { searchParams?: { lang?: string } };

export default function MergeLabPage({ searchParams }: MergeLabPageProps) {
  const locale = resolveLocale(searchParams?.lang);
  const summary = getMergeSummary();
  const signals = getMergeSignals();
  const tasks = getMergeTasks();
  ...
  return (
    <main className="min-h-screen bg-seis-bg text-seis-text">
      <section className="mx-auto w-[min(1120px,calc(100vw-1.5rem))] py-12 sm:py-16">
        <p className="text-xs uppercase tracking-[0.14em] text-seis-accent">SEIS Merge Lab</p>
        <h1 className="mt-3 max-w-4xl font-serif text-4xl leading-tight sm:text-5xl">Conflict hotspots, merge sequencing, and review depth in one command lane.</h1>
        <p className="mt-4 max-w-3xl text-sm text-seis-muted sm:text-base">Supports safer branch consolidation before integration waves hit protected delivery flows.</p>
        ...
        <div className="mt-6 grid gap-3 lg:grid-cols-2">
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Merge Signals</h2>
          ...
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Merge Tasks</h2>
            <ul className="mt-3 grid gap-2 text-sm text-seis-muted">
              {tasks.map((item) => (
                <li key={item.id} className="rounded border border-seis-line bg-[#1a1510] px-3 py-3">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Link href="/api/merge-command" className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">API: /api/merge-command</Link>
          <Link href={`/worktree-lab?lang=${locale}`} className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">Open worktree lab</Link>
