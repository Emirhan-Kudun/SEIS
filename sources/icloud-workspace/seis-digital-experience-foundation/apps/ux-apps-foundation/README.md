...
- `GET /api/motion-tokens`
- `GET /api/accessibility-affordances`
- `GET /api/experience-scorecard`
...
The motion token surface makes cinematic animation governable before adding heavier 3D dependencies. It exposes duration, easing, pressure, and fallback rules through `/api/motion-tokens` so future animation work can stay calm, reversible, and reduced-motion aware.

The experience scorecard keeps release quality visible without heavy tooling. It tracks accessibility, motion safety, mobile performance, content clarity, repository governance, and publish readiness through `/api/experience-scorecard`.
