(function () {
  ...
  function initActiveNavAndScrollState() {
    ...
    var navSectionMap = navLinks.map(function (link) {
      var hash = resolveHashAlias(link.getAttribute("href") || "");
      return {
        link: link,
        hash: hash,
    ...
    var sections = navSectionMap.map(function (entry) {
      return entry.section;
    });
    ...
    function update() {
      ...
      if (sections.length && navLinks.length) {
        ...
        navSectionMap.forEach(function (entry) {
          var isActive = entry.section.id === currentId;
  ...
  function setFieldInvalidState(input, invalid) {
    ...
    var errorNode = group ? q(".form-err", group) : null;
    if (group) {
      group.classList.toggle("invalid", !!invalid);
    }
    if (input) {
      input.setAttribute("aria-invalid", invalid ? "true" : "false");
      if (invalid && errorNode && errorNode.id) {
