...
`main` remains protected. `UIXAppTTR` is the canonical delivery branch. The former `codex/premium-local-foundation` scope is preserved as the `premium-local-foundation-agent` logical lane inside `UIXAppTTR`, not as a Git branch for new work.
...
SEIS-specific governance gates are also preserved:
...
The runtime-aware planner connects:

- MCP/skill/connector registry governance
- mission-aware activation rules
- live MCP readiness snapshots
- blocked-auth and permission-gated connector reporting
...
- `npm run check:automation-schedules` validates the low-power automation schedule registry and `/api/automation-schedules`.
- `npm run check:efficiency-mode` validates the token-heavy, machine-light operating profile and `/api/efficiency-mode`.
- `npm run check:github-publication` validates the iCloud-local-first and GitHub-server-push contract.
- `npm run check:github-server-sync` verifies local `UIXAppTTR` HEAD matches the GitHub server branch HEAD.
...
- `npm run check:repository-intake` validates the iCloud/GitHub repository intake registry before any external repo material is promoted.
- `npm run check:ci-workflow` validates the low-power CI workflow permissions, timeout, and focused command set.
...
Automation schedules are tracked in `docs/automation-schedules.md` and exposed locally through `/api/automation-schedules`.

Full-efficiency low-strain mode is tracked in `docs/efficiency-mode.md` and exposed locally through `/api/efficiency-mode`.

GitHub publication readiness is tracked in `docs/github-publication.md` and exposed locally through `/api/github-publication`.
...
- Canonical development branch: `UIXAppTTR`
- Absorbed local-foundation lane: `premium-local-foundation-agent`
