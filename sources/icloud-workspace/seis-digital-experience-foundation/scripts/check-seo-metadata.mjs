...
const caseStudyUrl = canonicalBase ? `${canonicalBase}/case-studies/seis-foundation.html` : null;

if (sitemapLocs.length < 1) {
  failures.push("sitemap must include at least the canonical home URL");
}
...
if (!homeHtml.includes('<meta name="robots" content="index, follow">')) {
  failures.push("home page must remain indexable in the current canonical publish mode");
}

if (!caseStudyHtml.includes('<meta name="robots" content="noindex, nofollow">')) {
  failures.push("case study page must remain noindex while it stays in draft mode");
}

if (caseStudyUrl && sitemapLocs.includes(caseStudyUrl)) {
  failures.push("sitemap must not include the draft noindex case study URL");
}
...
console.log(JSON.stringify({
  ...
  robotsMode: "home-indexable-case-study-draft"
}, null, 2));
