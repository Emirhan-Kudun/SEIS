...
async function handleReleaseReadiness(response) {
  ...
  sendJson(response, 200, {
    ...
    nextActions: remoteBlocked
      ? ["Run gh auth login -h github.com", "Rerun npm run publish:preflight", "Push codex/premium-local-foundation"]
      : ["Run npm run quality", "Run npm run publish:preflight", "Push codex/premium-local-foundation"]
  });
}
