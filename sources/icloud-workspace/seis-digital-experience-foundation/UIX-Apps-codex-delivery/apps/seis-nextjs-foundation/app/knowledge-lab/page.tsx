...
export const metadata: Metadata = {
  title: "Knowledge Lab",
  description: "Knowledge command surface for runbooks, documentation freshness, onboarding clarity, and governance traceability.",
  alternates: {
    canonical: "/knowledge-lab"
  }
};

type KnowledgeLabPageProps = {
  searchParams?: { lang?: string };
};

export default function KnowledgeLabPage({ searchParams }: KnowledgeLabPageProps) {
  const locale = resolveLocale(searchParams?.lang);
  const summary = getKnowledgeSummary();
  const signals = getKnowledgeSignals();
  const tasks = getKnowledgeTasks();
  ...
  return (
    <main className="min-h-screen bg-seis-bg text-seis-text">
      <section className="mx-auto w-[min(1120px,calc(100vw-1.5rem))] py-12 sm:py-16">
        <p className="text-xs uppercase tracking-[0.14em] text-seis-accent">SEIS Knowledge Lab</p>
        <h1 className="mt-3 max-w-4xl font-serif text-4xl leading-tight sm:text-5xl">
          Documentation and runbook command lane for resilient onboarding and governance continuity.
        </h1>
        <p className="mt-4 max-w-3xl text-sm text-seis-muted sm:text-base">
          Reduces architectural drift by keeping operational knowledge synchronized with system growth.
        </p>
        ...
        <div className="mt-6 grid gap-3 lg:grid-cols-2">
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Knowledge Signals</h2>
          ...
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Knowledge Tasks</h2>
            <ul className="mt-3 grid gap-2 text-sm text-seis-muted">
              {tasks.map((item) => (
                <li key={item.id} className="rounded border border-seis-line bg-[#1a1510] px-3 py-3">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Link href="/api/knowledge-command" className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">API: /api/knowledge-command</Link>
          ...
          <Link href={`/automation?lang=${locale}`} className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">Open automation</Link>
