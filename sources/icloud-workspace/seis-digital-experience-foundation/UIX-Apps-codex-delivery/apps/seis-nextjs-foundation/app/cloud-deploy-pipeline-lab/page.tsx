...
export const metadata: Metadata = {
  title: "Cloud Deploy Pipeline Lab",
  description: "Cloud deployment pipeline command surface for release gating, reproducibility, rollback SLAs, and verification depth.",
  alternates: {
    canonical: "/cloud-deploy-pipeline-lab"
  }
};

type CloudDeployPipelineLabPageProps = {
  searchParams?: { lang?: string };
};

export default function CloudDeployPipelineLabPage({ searchParams }: CloudDeployPipelineLabPageProps) {
  const locale = resolveLocale(searchParams?.lang);
  const summary = getCloudDeployPipelineSummary();
  const signals = getCloudDeployPipelineSignals();
  const tasks = getCloudDeployPipelineTasks();
  ...
  return (
    <main className="min-h-screen bg-seis-bg text-seis-text">
      <section className="mx-auto w-[min(1120px,calc(100vw-1.5rem))] py-12 sm:py-16">
        <p className="text-xs uppercase tracking-[0.14em] text-seis-accent">SEIS Cloud Deploy Pipeline Lab</p>
        <h1 className="mt-3 max-w-4xl font-serif text-4xl leading-tight sm:text-5xl">
          Cloud deployment planning, gating, rollback, and verification organized as a resilient delivery system.
        </h1>
        <p className="mt-4 max-w-3xl text-sm text-seis-muted sm:text-base">
          Converts pipeline risk into explicit release control checkpoints with predictable rollback behavior.
        </p>
        ...
        <div className="mt-6 grid gap-3 lg:grid-cols-2">
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Cloud Deploy Pipeline Signals</h2>
            <ul className="mt-3 grid gap-2 text-sm text-seis-muted">
              {signals.map((item) => (
                <li key={item.id} className="rounded border border-seis-line bg-[#1a1510] px-3 py-3">
                  <p className="text-xs uppercase tracking-[0.1em] text-seis-accent">{item.stage} - {item.status}</p>
          ...
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Cloud Deploy Pipeline Tasks</h2>
            <ul className="mt-3 grid gap-2 text-sm text-seis-muted">
              {tasks.map((item) => (
                <li key={item.id} className="rounded border border-seis-line bg-[#1a1510] px-3 py-3">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Link href="/api/cloud-deploy-pipeline-command" className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">API: /api/cloud-deploy-pipeline-command</Link>
          ...
          <Link href={`/release-lab?lang=${locale}`} className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">Open release lab</Link>
