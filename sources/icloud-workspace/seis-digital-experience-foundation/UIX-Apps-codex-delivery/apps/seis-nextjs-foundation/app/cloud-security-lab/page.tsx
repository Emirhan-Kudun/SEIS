...
export const metadata: Metadata = {
  ...
  description: "Cloud security command surface for IAM hardening, secret governance, network policy, and supply-chain integrity.",
  alternates: {
    canonical: "/cloud-security-lab"
  }
};

type CloudSecurityLabPageProps = {
  searchParams?: { lang?: string };
};

export default function CloudSecurityLabPage({ searchParams }: CloudSecurityLabPageProps) {
  const locale = resolveLocale(searchParams?.lang);
  const summary = getCloudSecuritySummary();
  const signals = getCloudSecuritySignals();
  const tasks = getCloudSecurityTasks();
  ...
  return (
    <main className="min-h-screen bg-seis-bg text-seis-text">
      <section className="mx-auto w-[min(1120px,calc(100vw-1.5rem))] py-12 sm:py-16">
        <p className="text-xs uppercase tracking-[0.14em] text-seis-accent">SEIS Cloud Security Lab</p>
        <h1 className="mt-3 max-w-4xl font-serif text-4xl leading-tight sm:text-5xl">
          Cloud identity, secrets, network, and supply-chain security unified in one operational command layer.
        </h1>
        <p className="mt-4 max-w-3xl text-sm text-seis-muted sm:text-base">
          Strengthens release trust with explicit security evidence and rollback-safe enforcement gates.
        </p>
        ...
        <div className="mt-6 grid gap-3 lg:grid-cols-2">
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Cloud Security Signals</h2>
          ...
          <article className="rounded-xl border border-seis-line bg-seis-surface p-4">
            <h2 className="font-serif text-3xl">Cloud Security Tasks</h2>
            <ul className="mt-3 grid gap-2 text-sm text-seis-muted">
              {tasks.map((item) => (
                <li key={item.id} className="rounded border border-seis-line bg-[#1a1510] px-3 py-3">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Link href="/api/cloud-security-command" className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">API: /api/cloud-security-command</Link>
          ...
          <Link href={`/cloud-lab?lang=${locale}`} className="rounded-xl border border-seis-line bg-seis-surface p-4 text-sm text-seis-muted hover:border-seis-accent">Open cloud lab</Link>
