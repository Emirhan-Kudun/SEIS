(function () {
  ...
  var ROADMAP_API_PATH = "/api/roadmap";
  var DEVELOPMENT_PROGRAM_API_PATH = "/api/development-program";
  var CINEMATIC_DEPTH_API_PATH = "/api/cinematic-depth";
  ...
  var FOCUS_STORAGE_KEY = "ek_focus_mode";
  ...
  var rootClassName = (document.documentElement.className || "")
    .replace(/(^|\s)no-js(?=\s|$)/, " ")
    .replace(/\s+/g, " ")
    .replace(/^\s|\s$/g, "");
  document.documentElement.className = (rootClassName ? rootClassName + " " : "") + "js";
  ...
  function readStoredFocusMode() {
    try {
      var stored = String(window.localStorage.getItem(FOCUS_STORAGE_KEY) || "").toLowerCase();
      return stored === "calm" ? "calm" : "standard";
    } catch (err) {
      return "standard";
    }
  }

  function applyFocusMode(mode, options) {
    var settings = options || {};
    var normalized = String(mode || "").toLowerCase() === "calm" ? "calm" : "standard";
    var active = normalized === "calm";
    var button = q("#focus-toggle");
    userFocusMode = normalized;
    document.documentElement.setAttribute("data-focus", normalized);
    document.body.setAttribute("data-focus", normalized);
    if (button) {
      button.classList.toggle("active", active);
      button.setAttribute("aria-pressed", active ? "true" : "false");
      button.setAttribute("aria-label", getT(active ? "focus.aria.on" : "focus.aria.off", currentLang) || (active ? "Calm focus mode enabled" : "Calm focus mode"));
    }
    if (settings.persist !== false) {
      try {
        window.localStorage.setItem(FOCUS_STORAGE_KEY, normalized);
      } catch (err) {
        /* noop */
      }
    }
    if (settings.toast) {
      showToast(getT(active ? "focus.toast.on" : "focus.toast.off", currentLang) || (active ? "Calm mode enabled." : "Calm mode disabled."));
    }
  }
  ...
  function getLanguageLabel(lang, uiLang) {
    var safeLang = normalizeLang(lang);
    if (!safeLang) {
      return String(lang || "").toUpperCase();
    }

    var preferredUiLang = normalizeLang(uiLang) || currentLang || DEFAULT_LANG;
    var key = "lang.name." + safeLang;
    var preferredValue = getT(key, preferredUiLang);
    if (typeof preferredValue === "string" && preferredValue.trim()) {
      return preferredValue;
    }

    if (translations && translations[safeLang] && Object.prototype.hasOwnProperty.call(translations[safeLang], key)) {
      var nativeValue = translations[safeLang][key];
      if (typeof nativeValue === "string" && nativeValue.trim()) {
        return nativeValue;
      }
    }

    return NATIVE_LANGUAGE_LABELS[safeLang] || safeLang.toUpperCase();
  }
  ...
  function updateLanguageButtons(uiLang) {
    qa(".lang-btn").forEach(function (btn) {
      var lang = normalizeLang(btn.getAttribute("data-lang"));
      if (!lang) {
        return;
      }
      var label = getLanguageLabel(lang, lang);
  ...
  function updateLanguageChip(lang) {
    ...
    var label = getLanguageLabel(safeLang, safeLang);
    var ariaTemplate = getT("ui.lang.current", safeLang) || "Active language: {lang}";
    chip.textContent = label;
    chip.setAttribute("title", label);
    chip.setAttribute("lang", safeLang);
    chip.setAttribute("aria-label", applyTemplate(ariaTemplate, { lang: label }));
    var trigger = q("#lang-trigger");
    if (trigger) {
      trigger.setAttribute("aria-label", applyTemplate(ariaTemplate, { lang: label }));
    }
  }

  function setLanguageMenuOpen(open) {
    var switcher = q("#lang-switcher");
    var trigger = q("#lang-trigger");
    if (!switcher || !trigger) {
      return;
    }
  ...
  function initLanguageSwitcher() {
    var switcher = q("#lang-switcher");
    var trigger = q("#lang-trigger");
    var menu = q("#lang-menu");
    var selector = menu ? "#lang-menu .lang-btn" : ".lang-btn";

    updateLanguageButtons(currentLang);
    initChoiceGroupKeyboard(selector);
    ...
    if (switcher && trigger && menu) {
      ...
      trigger.addEventListener("click", function () {
        ...
        if (!open) {
          var activeBtn = q("#lang-menu .lang-btn.active") || q("#lang-menu .lang-btn");
          if (activeBtn) {
            activeBtn.focus();
          }
        }
    ...
    qa(selector).forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        if (lang && lang !== currentLang) {
          ...
          showToast(applyTemplate(messageTemplate, { lang: getLanguageLabel(lang, lang) }));
  ...
  function initFocusToggle() {
    ...
    button.addEventListener("click", function () {
      var nextMode = userFocusMode === "calm" ? "standard" : "calm";
      applyFocusMode(nextMode, { persist: true, toast: true });
      trackMetric("focus_mode", { mode: nextMode });
    });
    onLanguageChange(function () {
      applyFocusMode(userFocusMode, { persist: false });
    });
  }

  function initFullstackRoadmap() {
    var grid = q("#roadmap-grid");
    var status = q("#roadmap-status");
    if (!grid || !status || !window.fetch) {
      return;
    }

    function setStatus(key) {
      var value = getT(key, currentLang);
      if (typeof value === "string") {
    ...
    function createRoadmapCard(track) {
      var stateClass = track.status === "active" ? " roadmap-state-active" : "";
      var stateLabel = track.statusLabel || getT("plan.state." + track.status, currentLang) || track.status || "planned";
      var card = document.createElement("article");
      card.className = "roadmap-card";
      card.setAttribute("role", "listitem");
      card.appendChild(createTextElement("p", "roadmap-state" + stateClass, stateLabel));
      card.appendChild(createTextElement("h3", "", track.title || ""));
      card.appendChild(createTextElement("p", "", track.detail || ""));
      return card;
    }

    function renderRoadmap(payload) {
      if (!payload || !Array.isArray(payload.tracks) || payload.tracks.length === 0) {
        setStatus("plan.status.fallback");
        return;
      }
      var fragment = document.createDocumentFragment();
      payload.tracks.forEach(function (track) {
        fragment.appendChild(createRoadmapCard(track));
      });
      grid.replaceChildren(fragment);
      setStatus("plan.status.ready");
    }

    function loadRoadmap() {
      setStatus("plan.status.loading");
      fetch(ROADMAP_API_PATH + "?lang=" + encodeURIComponent(currentLang), { cache: "no-cache" })
    ...
    loadRoadmap();
    onLanguageChange(loadRoadmap);
  }

  function initDevelopmentCockpit() {
    var root = q("#development-cockpit");
    var title = q("#development-sprint-title");
    var score = q("#development-score");
    var lanesRoot = q("#development-lanes");
    var note = q("#development-note");
    if (!root || !title || !score || !lanesRoot || !note || !window.fetch) {
      return;
    }
    ...
    function createLane(lane) {
      ...
      var status = String(lane.status || "planned").toLowerCase();
      card.className = "development-lane";
      card.setAttribute("role", "listitem");
      state.className = "development-lane-state is-" + status;
      state.textContent = status;
      heading.textContent = lane.label || lane.id || "Lane";
      copy.textContent = lane.focus || lane.nextAction || "";
    ...
    fetch(DEVELOPMENT_PROGRAM_API_PATH, { cache: "no-cache" })
      .then(function (response) {
        if (!response.ok) {
          throw new Error("development program api unavailable");
        }
        return response.json();
      })
      .then(function (payload) {
        var sprint = payload.currentSprint || {};
        var quality = (payload.signals || {}).quality || {};
        var serverTarget = ((payload.signals || {}).serverTarget || {});
        var lanes = Array.isArray(payload.lanes) ? payload.lanes : [];
        title.textContent = sprint.title || "Development Program";
        score.textContent = typeof quality.overallScore === "number" ? quality.overallScore + "/100" : "--";
        lanesRoot.replaceChildren.apply(lanesRoot, lanes.map(createLane));
        note.textContent = serverTarget.missingEnvironment && serverTarget.missingEnvironment.length
          ? "Server upload bekliyor: " + serverTarget.missingEnvironment.join(", ")
          : "Development program local olarak yayına hazır.";
      })
      .catch(function () {
        note.textContent = "Development program şu an local fallback ile bekliyor.";
      });
  }

  function initDevelopmentDepthCanvas() {
    var canvas = q("#development-depth-canvas");
    var cockpit = q("#development-cockpit");
    if (!canvas || !cockpit || !window.fetch || isReducedMotion()) {
      return;
    }
    ...
    function applyAdaptiveLowPowerSettings() {
      if (!isLowPowerDevice()) {
        return;
      }
      settings.particleCount = Math.min(settings.particleCount, 16);
      settings.maxDevicePixelRatio = Math.min(settings.maxDevicePixelRatio, 1.25);
      settings.frameIntervalMs = Math.max(settings.frameIntervalMs, 96);
      settings.opacity = Math.min(settings.opacity, 0.26);
    }

    function canAnimate() {
      return depthReady && !document.hidden && inViewport && !isReducedMotion();
    }
    ...
    function resize() {
      ...
      var ratio = Math.min(window.devicePixelRatio || 1, settings.maxDevicePixelRatio || 1);
    ...
    function seedParticles() {
      var rect = cockpit.getBoundingClientRect();
      particles = [];
      for (var index = 0; index < settings.particleCount; index += 1) {
        var layer = index % settings.depthLayers;
        particles.push({
          x: (index * 89 % 997) / 997 * rect.width,
          y: (index * 233 % 991) / 991 * rect.height,
          r: 1.2 + layer * 0.65,
          layer: layer + 1,
          drift: 0.08 + layer * 0.035
        });
      }
    }
    ...
    function draw(timestamp) {
      ...
      var rect = cockpit.getBoundingClientRect();
      if (timestamp - lastFrame < settings.frameIntervalMs) {
        queueAnimation();
        return;
      }
      lastFrame = timestamp;
      ...
      particles.forEach(function (particle) {
        ...
        if (particle.x > rect.width + 8) {
          particle.x = -8;
        }
        context.beginPath();
        context.fillStyle = particle.layer === 1 ? "rgba(255,255,255,.42)" : "rgba(200,169,110,.52)";
    ...
    function setupViewportObserver() {
      ...
      var observer = new window.IntersectionObserver(function (entries) {
        ...
        inViewport = !!(entry && entry.isIntersecting && entry.intersectionRatio > 0.04);
        if (inViewport) {
          queueAnimation();
        } else {
          stopAnimation();
    ...
    fetch(CINEMATIC_DEPTH_API_PATH, { cache: "no-cache" })
      .then(function (response) {
        if (!response.ok) {
          throw new Error("cinematic depth api unavailable");
        }
        return response.json();
      })
      .then(function (payload) {
        settings = Object.assign(settings, payload.settings || {});
        applyAdaptiveLowPowerSettings();
  ...
  function initCookieBar() {
    ...
    if (!stored) {
      window.setTimeout(function () { cookieBar.classList.add("visible"); }, 1400);
    }
  ...
  function initLazyMediaViewportTrigger() {
    ...
    // Viewport trigger for non-iframe media blocks
    var observer = new IntersectionObserver(function (entries, io) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
  ...
  function initCursor() {
    ...
    if (!FINE_POINTER || isReducedMotion()) {
  ...
  function init() {
    ...
    userFocusMode = readStoredFocusMode();
    ...
    initMotionSwitcher();
    initFocusToggle();
    initFullstackRoadmap();
    initDevelopmentCockpit();
    initDevelopmentDepthCanvas();
    ...
    loadTranslations()
      ...
      .catch(function () {
        ...
        updateLanguageButtons(resolved.lang);
