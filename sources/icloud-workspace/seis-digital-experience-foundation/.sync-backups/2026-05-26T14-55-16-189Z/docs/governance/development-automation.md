...
- Publish readiness uses quick low-pressure checks by default. Use
  `SEIS_PUBLISH_READINESS_FULL=1 npm run automation:publish-readiness` only
  when a final full preflight is needed.
