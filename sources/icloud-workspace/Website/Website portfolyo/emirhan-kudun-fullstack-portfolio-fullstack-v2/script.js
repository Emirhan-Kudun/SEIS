(function () {
  ...
  var AVAILABILITY_API_PATH = "/api/availability";
  var BRIEF_API_PATH = "/api/brief";
  ...
  function initFullstackRoadmap() {
    ...
    function renderRoadmap(payload) {
      ...
      grid.innerHTML = payload.tracks.map(function (track) {
        ...
        return [
          '<article class="roadmap-card" role="listitem">',
          '<p class="roadmap-state' + stateClass + '">' + escapeHtml(stateLabel) + '</p>',
  ...
  function initLiveAvailability() {
    var label = q(".availability-badge [data-i18n='ct.availability']");
    ...
    function renderAvailability(payload) {
      ...
      lastPayload = payload;
      var key = payload.openSlots > 0 ? "availability.open" : "availability.limited";
      var template = getT(key, currentLang);
      if (typeof template !== "string") {
        return;
      }
      label.textContent = applyTemplate(template, {
        slots: payload.openSlots,
        hours: payload.responseHours
      });
    }

    fetch(AVAILABILITY_API_PATH, { cache: "no-cache" })
      .then(function (response) {
        if (!response.ok) {
          throw new Error("availability api unavailable");
        }
        return response.json();
      })
      .then(renderAvailability)
      .catch(function () {
        /* Static availability copy remains in place. */
      });

    onLanguageChange(function () {
      if (lastPayload) {
        renderAvailability(lastPayload);
      }
    });
  }

  function initBriefIntelligence() {
    var form = q("#contact-form");
    var button = q("#brief-ai-btn");
    var result = q("#brief-ai-result");
    var status = q("#brief-ai-status");
    var score = q("#brief-ai-score");
    var timeline = q("#brief-ai-timeline");
    var complexity = q("#brief-ai-complexity");
    var steps = q("#brief-ai-steps");
    if (!form || !button || !result || !status || !score || !timeline || !complexity || !steps || !window.fetch) {
      return;
    }
    ...
    function getBriefLabel(prefix, value) {
      return getT(prefix + "." + value, currentLang) || String(value || "");
    }

    function renderBrief(payload) {
      if (!payload || payload.ok !== true) {
        setBriefStatus("brief.ai.error");
        return;
      }
      lastPayload = payload;
      result.hidden = false;
      score.textContent = String(payload.readinessScore || 0) + "%";
      timeline.textContent = applyTemplate(getT("brief.timeline.weeks", currentLang) || "{weeks} wk", {
        weeks: payload.timelineWeeks || 1
      });
      complexity.textContent = getBriefLabel("brief.complexity", payload.complexity);
      steps.innerHTML = (payload.nextSteps || []).map(function (step) {
        return "<li>" + escapeHtml(getBriefLabel("brief.step", step)) + "</li>";
      }).join("");
      setBriefStatus("brief.ai.ready");
    }
    ...
    button.addEventListener("click", function () {
      ...
      setBriefStatus("brief.ai.loading");
      fetch(BRIEF_API_PATH, {
        method: "POST",
        headers: {
        ...
        .then(function (response) {
          if (!response.ok) {
            throw new Error("brief api unavailable");
          }
          return response.json();
        })
        .then(function (data) {
          renderBrief(data);
          trackMetric("brief_analysis", {
    ...
    onLanguageChange(function () {
      if (lastPayload) {
        renderBrief(lastPayload);
      } else {
        setBriefStatus("brief.ai.idle");
      }
    });
  }
