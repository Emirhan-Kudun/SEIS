...
import type { Locale, LocalizedDictionary, PortfolioIndexItem } from "@seis/content";
...
function resolvePortfolioHref(item: PortfolioIndexItem, locale?: Locale) {
  if (item.external || !locale || !item.href.startsWith("/")) {
    return item.href;
  }

  return withLocalePath(locale, item.href);
}

export function PortfolioIndex({ dictionary, items, compact = false, locale }: PortfolioIndexProps) {
        ...
        <div className="portfolio-index-grid">
          {visibleItems.map((item) => (
            <a
              ...
              href={resolvePortfolioHref(item, locale)}
              key={item.id}
