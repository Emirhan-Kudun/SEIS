...
type PortfolioCollectionsProps = {
  ...
  compact?: boolean;
  hrefs?: Partial<Record<PortfolioCollectionItem["id"], string>>;
};

export function PortfolioCollections({ dictionary, collections, compact = false, hrefs = {} }: PortfolioCollectionsProps) {
    ...
    <section className="portfolio-collections" data-compact={compact} aria-labelledby="portfolio-collections-title">
      ...
      <div className="portfolio-collection-grid">
        {visibleCollections.map((collection) => {
          const href = hrefs[collection.id] || collection.href;

          return (
            <article className="portfolio-collection-card" data-accent={collection.accent} key={collection.id}>
              <a className="portfolio-collection-media" href={href} aria-label={`${dictionary.portfolioCollectionOpen}: ${collection.title}`}>
              ...
              <div className="portfolio-collection-copy">
                ...
                <a className="text-link" href={href}>{dictionary.portfolioCollectionOpen}</a>
              </div>
            </article>
          );
        })}
