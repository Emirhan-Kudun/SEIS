...
import type { DigitalLabEngagement, LocalizedDictionary, ServiceItem } from "@seis/content";
...
export function BriefIntakeForm({
  ...
  const selectedEngagement = useMemo(
    () => engagements.find((engagement) => engagement.id === selectedEngagementId) ?? engagements[0],
      ...
      <fieldset className="brief-fieldset" disabled={isSending}>
        ...
        <div className="form-grid">
          ...
          {engagements.length > 0 && (
            <label>
              <span>{dictionary.briefEngagement}</span>
              <select
                name="engagement"
                value={selectedEngagement?.id ?? ""}
                onChange={(event) => setSelectedEngagementId(event.currentTarget.value)}
              >
        ...
        {selectedEngagement && (
          <aside className="brief-engagement-summary" aria-live="polite">
            <div>
              <span>{selectedEngagement.pace}</span>
              <strong>{selectedEngagement.title}</strong>
              <p>{selectedEngagement.bestFor}</p>
            </div>
            <ul>
              {selectedEngagement.scope.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
            <p>{selectedEngagement.qualityGate}</p>
          </aside>
        )}
        <label>
        ...
        <label>
          ...
          <textarea name="message" required minLength={12} rows={5} />
        </label>
