...
const navItems = [
  ...
  ["/portfolio-os", "navPortfolioOs", "portfolio-os"],
  ["/connector-console", "navConnectorConsole", "connector-console"],
  ["/platform-adapters", "navPlatformAdapters", "platform-adapters"],
  ["/cloud-environment", "navCloudEnvironment", "cloud-environment"],
  ["/server-handoff", "navServerHandoff", "server-handoff"],
  ["/security-posture", "navSecurityPosture", "security-posture"],
  ["/studio-crm", "navStudioCrm", "studio-crm"],
  ["/brief-triage", "navBriefTriage", "brief-triage"],
  ["/publishing-console", "navPublishingConsole", "publishing-console"],
  ["/case-study-builder", "navCaseStudyBuilder", "case-study-builder"],
  ["/portfolio#portfolio-behance", "navBehance", "behance"],
  ["/drawings", "navDrawings", "drawings"],
  ["/lab", "navLab", "lab"],
  ["/design-system", "navDesign", "design"],
  ["/readiness", "navReadiness", "readiness"],
  ["/motion", "navMotion", "motion"],
  ["/motion-presets", "navMotionPresets", "motion-presets"],
  ["/os", "navSupremeOs", "os"],
  ["/compatibility", "navCompatibility", "compatibility"],
  ["/cv", "navExperience", "cv"],
  ["/contact", "navContact", "contact"]
] as const;

export function PageSurface({ mode, locale }: { mode: PageMode; locale: Locale }) {
      ...
      <nav className="top-nav" aria-label={dict.primaryNavigationLabel}>
        ...
        <div className="nav-links">
          {navItems.map(([href, key, itemMode]) => (
            <Link key={href} href={withLocalePath(locale, href)} aria-current={itemMode === mode ? "page" : undefined}>
              {dict[key]}
            </Link>
          ))}
        </div>
      ...
      {mode === "portfolio" && (
        <section className="section page-hero" id="page-content">
          ...
          <PortfolioCollections
            ...
            hrefs={{
              "behance-campaigns": "#portfolio-behance",
              "drawing-archive": withLocalePath(locale, "/drawings"),
              "cinematic-web": withLocalePath(locale, "/motion"),
              "identity-editorial": withLocalePath(locale, "/#services")
            }}
          />
          <PortfolioDiscoveryFlow
            ...
            drawings={drawings}
            works={works}
            links={{ behance: "#portfolio-behance", drawings: withLocalePath(locale, "/drawings"), works: "#portfolio-work-cards" }}
          />
          ...
          <PortfolioIndex dictionary={dict} items={portfolioIndex} locale={locale} />
          ...
          <div className="card-grid" id="portfolio-work-cards">
            {works.map((work) => (
              <article className="work-card" key={work.id}>
                ...
                <Link className="text-link" href={withLocalePath(locale, `/portfolio/${work.id}`)}>{dict.portfolioOpenItem}</Link>
              </article>
            ))}
          </div>
        </section>
      ...
      {mode === "contact" && (
        <section className="section page-hero contact-page" id="page-content">
          ...
          <div className="contact-layout">
            ...
            <BriefIntakeForm dictionary={dict} engagements={digitalLabEngagements} services={services} />
          </div>
        </section>
      )}
    </main>
