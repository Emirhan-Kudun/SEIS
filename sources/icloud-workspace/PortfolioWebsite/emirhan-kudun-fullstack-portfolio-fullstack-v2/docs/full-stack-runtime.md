...
- `GET /api/roadmap` returns the multilingual full-stack roadmap used by the Plan section.
- `GET /api/i18n-health` reports translation key coverage for TR/EN/FR/IT/DE.
- `GET /api/availability` returns the public project-slot and response-window signal.
- `POST /api/brief` analyzes service/message fields and returns readiness, timeline, complexity, and next-step signals.
